...
import { Card, CardSection, Input, Button, Spinner, DropDown } from "./common";
import { createUser } from "../actions";

class RegisterTherapist extends Component {
  constructor(props) {
    super(props);
    this.params = {
      username: "",
  ...
  registerButtonPressed() {
    const { username, first_name, last_name, occupation_id } = this.params;
    ...
    if (!this.props.loading) {
      return (
        <Card>
          ...
          <CardSection>
            <DropDown
              url="http://10.0.2.2:5000/occupations"
              onValueChange={value =>
                this.onValueChange(value, "occupation_id")
              }
            />
          </CardSection>
          {this.renderError()}
...
export default connect(mapStateToProps, { createUser })(RegisterTherapist);
